...
void Q2RLE(float* QMatrix, int blocksize, float* runlengthvect, int* sizeRunL){
...
void encode(float* coeff, float* imgblock, float* imgDCT, float* Temporal, int blocksize,int* sizeRL,float* runlengthvect){
    DCT(coeff, imgblock, imgDCT, Temporal, blocksize);  //Applying DCT 
    Q(imgDCT,Temporal,blocksize);                       //Quantization
    Q2RLE(Temporal,blocksize,runlengthvect, sizeRL);
}
void decode(float* coeff2,float* imgblock,float* imgDCT, int blocksize, int size, float* RLE, int DCVal){
    float Temporal[blocksize][blocksize];
    RLE2Q(*Temporal, RLE, size,blocksize,DCVal);
...
int main(){
    ...
    for (x=0;  x<width/blocksize; x++){
        for (y=0;  y<length/blocksize; y++){
            ...
            encode(*coeff, *imgblock, *imgDCT, *Temporal, blocksize, &sizeRL, *runlenvect);
            decode(*coeff2,*imgblock, *imgDCT, blocksize,sizeRL,*runlenvect,**Temporal);
